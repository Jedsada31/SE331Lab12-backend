...
public class CloudStorageHelper {
    ...
    static {
        ...
        try {
            serviceAccount = new ClassPathResource("Private_Key.json").getInputStream();
            storage = StorageOptions.newBuilder().setCredentials(GoogleCredentials.fromStream(serviceAccount)).setProjectId("imageupload-9eed8").build().getService();
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        BlobInfo blobInfo =
                storage.create(
                        BlobInfo
                                ...
                                .setAcl(new ArrayList<>(Arrays.asList(Acl.of(Acl.User.ofAllUsers(), Acl.Role.READER)))).build(), os.toByteArray());
